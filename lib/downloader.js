    ...
    Downloader.prototype.login = function(callback) {
      return spotifyWeb.login(this.config.username, this.config.password, (function(_this) {
        return function(err, SpotifyInstance) {
          ...
          Logger.Log("Checking for Spotify Premium subscription error...");

          // check if we will have issue downloading tracks due to 'Error: Account subscription status not Spotify Premium' which happens intermittently
          SpotifyInstance.get('spotify:track:5ChkMS8OtdzJeqyybCc9R5', function (err, track) {
            if (err) {
              return Logger.Error("Failed to check for Spotify Premium subscription error." + err);
            }
            // attempt to play the track to test for the issue
            var stream = track.play()
            stream.on('error', function(err) {
              if (err.message.indexOf('Premium') > -1) {
                Logger.Log("Spotify Premium subscription error found! Trying to log in again...");
                if (_this.loginRetry > 10) {
            ...
            stream.on('readable', function() {
              Logger.Log("Spotify Premium subscription check finished.");
